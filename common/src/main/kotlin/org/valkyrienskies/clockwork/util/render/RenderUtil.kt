...
object RenderUtil {
     ...
     */
    fun renderCubeMatrix(matrices: PoseStack,
    ...
    fun renderCube(matrices: PoseStack,
                   renderer: PartialItemModelRenderer,
                   data: TransformData,
                   scale: Float,
                   light: Int) {
     ...
     */
    fun renderAndTransform(matrices: PoseStack,
                           model: PartialModel,
                           renderType: RenderType,
                           renderer: PartialItemModelRenderer,
                           modelCorrection: org.joml.Vector3f,
                           offset: org.joml.Vector3f,
                           rotationVec: org.joml.Vector3f,
                           scale: Float,
                           light: Int) {
        matrices.pushPose()
        ...
        matrices.translate(-modelCorrection.x().toDouble(),
            -modelCorrection.y().toDouble(),
            -modelCorrection.z().toDouble())
    ...
    fun renderCubeMatrix(matrices: PoseStack,
        ...
        renderAndTransform(crystal_inner, scale, innerData.offset, innerData.rotation).light(light)
            .color(255, 255, 255, 255).overlay().disableDiffuse().renderInto(matrices, crystal_inner_buffer)
        ...
        renderAndTransform(crystal, scale, data.offset, data.rotation).light(light).color(255, 255, 255, 255).overlay()
            .disableDiffuse().renderInto(matrices, crystal_buffer)
        ...
        renderAndTransform(crystal_outer, scale, outerData.offset, outerData.rotation).light(light)
            .color(255, 255, 255, 255).overlay().renderInto(matrices, crystal_outer_buffer)
    }
     ...
     * Transforms and renders a model
     */
    private fun renderAndTransform(buffer: SuperByteBuffer,
                                   scale: Float,
                                   coreOffset: org.joml.Vector3f,
                                   coreRotation: org.joml.Vector3f): SuperByteBuffer {
        //Scale
        buffer.scale(scale)
        buffer.translate(-(1 / (scale.toDouble() * 4)), -(1 / (scale.toDouble() * 4)), -(1 / (scale.toDouble() * 4)))
        //Y
        buffer.translateY((coreOffset.y * 2).toDouble())
            .rotateCentered(Direction.UP, (coreRotation.y / 180 * Math.PI).toFloat())
        //Z
        buffer.translateY((coreOffset.z * 2).toDouble())
            .rotateCentered(Direction.NORTH, (coreRotation.z / 180 * Math.PI).toFloat())
        //X
        buffer.translateY((coreOffset.x * 2).toDouble())
            .rotateCentered(Direction.EAST, (coreRotation.x / 180 * Math.PI).toFloat())

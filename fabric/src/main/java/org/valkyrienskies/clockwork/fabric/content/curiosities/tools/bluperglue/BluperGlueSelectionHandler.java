...
public class BluperGlueSelectionHandler {
    ...
    public boolean onMouseInput(boolean attack) {
        ...
        if (attack) {
            if (selected == null)
                return false;
            AllClockworkPackets.channel.sendToServer(new BluperGlueRemovalPacket(selected.getId(), soundSourceForRemoval));
            selected = null;
            clusterCooldown = 0;
            return true;
        }
    ...
    public void confirm() {
        LocalPlayer player = Minecraft.getInstance().player;
        AllClockworkPackets.channel.sendToServer(new BluperGlueSelectionPacket(firstPos, hoveredPos));

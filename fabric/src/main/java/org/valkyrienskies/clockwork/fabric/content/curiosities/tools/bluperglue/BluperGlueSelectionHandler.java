...
public class BluperGlueSelectionHandler {
    ...
    public boolean onMouseInput(boolean attack) {
        ...
        if (attack) {
            if (selected == null)
                return false;
            FabricClockworkPackets.channel.sendToServer(new BluperGlueRemovalPacket(selected.getId(), soundSourceForRemoval));
            selected = null;
            clusterCooldown = 0;
            return true;
        }
    ...
    public void confirm() {
        LocalPlayer player = Minecraft.getInstance().player;
        FabricClockworkPackets.channel.sendToServer(new BluperGlueSelectionPacket(firstPos, hoveredPos));
